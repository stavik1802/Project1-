...
class RunningNormalizer:
    ...
    def normalize(self,data,center=True):
        ...
        if isinstance(data, torch.Tensor):
            if isinstance(self.mean_active, np.ndarray):
                mean_active = torch.tensor(self.mean_active, dtype=data.dtype, device=data.device)
            else:
                mean_active = self.mean_active
            
            std_clamped = torch.clamp(torch.tensor(self.std_active, dtype=data.dtype, device=data.device), min=1e-8)
        
        else:
            std_clamped = np.maximum(self.std_active,1e-8)
            mean_active = self.mean_active
        #stav added 
        ...
        if center:
            stand = (data - mean_active) / std_clamped
        else:
            stand = data / std_clamped
    ...
    def denormalize(self, data_norm,center=True):
        ...
        if isinstance(data_norm, torch.Tensor):
            std_clamped = torch.clamp(torch.tensor(self.std_active, dtype=data_norm.dtype, device=data_norm.device), min=1e-8)
            mean_active = torch.tensor(self.mean_active, dtype=data_norm.dtype, device=data_norm.device)
        else:
            std_clamped = np.maximum(self.std_active,1e-8)
            mean_active = self.mean_active
            
        if center:
            data = data_norm * std_clamped + mean_active
        else:
            data = data_norm * std_clamped
...
class RunningNormalizers:
    ...
    def __init__(self,s_dim,a_dim,gamma,init_rms_stats=None, r_dim=None):
        ...
        self.ret_rms = RunningNormalizer(r_dim)                 # reward returns
