...
class Logger:
    ...
    def dump(self):
        ...
        for k,v in self.train_dict.items():
            if isinstance(v, torch.Tensor):  # Check if v is a PyTorch tensor
                train_out[k] = v.cpu().detach().numpy()  # Move to CPU, detach, and convert to NumPy
            elif isinstance(v, list):  # Handle lists of tensors or other values
                train_out[k] = np.array([item.cpu().detach().numpy() if isinstance(item, torch.Tensor) else item for item in v])
            else:  # Handle other data types as is
                train_out[k] = v
