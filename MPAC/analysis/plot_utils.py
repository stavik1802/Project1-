...
def create_plot(x_list, metrics_list,
    ...
    for plot_idx in range(tot):
        ...
        # Plot all raw curves (light lines)
        if show_all:
            for file_idx in range(len(metrics_list)):
                ...
                for datum in data_active:
                    ax_active.plot(x_active, datum, color=f'C{file_idx}', alpha=0.5)

        # Plot mean and shaded area
        for file_idx in range(len(metrics_list)):
            x_active = x_list[file_idx][tot_active]
            data_active = metrics_list[file_idx][tot_active]
            label_active = labels[file_idx] if labels else f'File {file_idx}'

            data_mean = np.mean(data_active, axis=0)
            ax_active.plot(x_active, data_mean, color=f'C{file_idx}', label=label_active)
            ...
            if se_val > 0.0:
                if len(data_active.shape) == 3:
                    for datum_idx in range(data_active.shape[-1]):
                        ...
                        ax_active.fill_between(x_active, data_low, data_high, color=f'C{file_idx}', alpha=0.2)
        ...
        if row_active == rows - 1:
            ax_active.set_xlabel(x_label or title)

        # Dynamically map y-labels from metric name
        metric_name = metrics_list[0][plot_idx].name if hasattr(metrics_list[0][plot_idx], 'name') else ""
        metric_label_dict = {
            "J_tot": "Total Reward",
            "Jc_tot": "Total Cost",
            "J_disc": "Discounted Reward",
            "Jc_disc": "Discounted Cost",
            "J_tot_eval_nominal": "Total Eval Reward",
            "Jc_tot_eval_nominal": "Total Eval Cost",
            "ent": "Entropy",
            "critic_loss": "Critic Loss",
        }
        ylabel = metric_label_dict.get(titles[plot_idx] if titles else metric_name, "Value")
        ax_active.set_ylabel(ylabel)

        if row_active == 0 and col_active == 0:
            handles, legend_labels = ax_active.get_legend_handles_labels()
            temp = {k: v for k, v in zip(legend_labels, handles)}
            ax_active.set_title(title)
...
#         title = titles[0]
#         if row_active == rows-1:
#             # label_dict = {"Cartpole Swingup":"Pole Length","Walker Walk":"Torso Length",
#             #               "Walker Run":"Torso Length","Quadruped Walk":"Torso Density",
#             #               "Quadruped Run":"Torso Density"}
#             # ax_active.set_xlabel(label_dict[title])
#             metric_name = metrics_list[0][plot_idx].shape[-1] if isinstance(metrics_list[0][plot_idx],
#                                                                             np.ndarray) else plot_idx
#             metric_key = titles[0] if isinstance(titles, list) else 'J_tot'  # fallback
#             ylabel = label_dict.get(metric_key, metric_key)
#             ax_active.set_ylabel(ylabel)
#         if col_active == 0:
#             # label = ["Total Reward", "Total Cost"]
#             # ax_active.set_ylabel(label[tot_active])
#             metric_name = metrics_list[0][plot_idx].shape[-1] if isinstance(metrics_list[0][plot_idx],
#                                                                             np.ndarray) else plot_idx
#             metric_key = titles[0] if isinstance(titles, list) else 'J_tot'  # fallback
#             ylabel = label_dict.get(metric_key, metric_key)
...
#         list(temp.values()),
#         list(temp.keys()),
#         bbox_to_anchor=(0.5, -0.1),  # Adjust this value to position the legend
#         loc='upper center',
