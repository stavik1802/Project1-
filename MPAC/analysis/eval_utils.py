...
def set_perturb_defaults(import_logs, inputs_dict):
    ...
    if (perturb_param_min is None) and (perturb_param_max is None):
        try:
            env_kwargs = import_logs[0]['param']['env_kwargs']
            domain = env_kwargs['env_name'].replace('_', '')  # ← normalize
            rwrl_defaults = rwrl_defaults_eval[domain]
            setup_kwargs.update(rwrl_defaults)
        except KeyError:
            raise ValueError(f"Unknown domain '{domain}' — must define defaults in rwrl_defaults_eval")
        except:
...
def evaluate(env, actor, gamma=1.00, env_horizon=48, num_traj=5, deterministic=True, seed=0):
    init_seeds(seed, [env])
...
rwrl_defaults_eval_energynet = {'perturb_param_name': 'battery', 'perturb_param_min': 0.8, 'perturb_param_max': 1.2}

rwrl_defaults_eval = {
    'cartpole': rwrl_defaults_eval_cartpole,
    'walker': rwrl_defaults_eval_walker,
    'quadruped': rwrl_defaults_eval_quadruped,
    'energynet': rwrl_defaults_eval_energynet
}
...
rwrl_defaults_nominal = {
    ...
    'energynet': rwrl_defaults_nominal_energynet
}
