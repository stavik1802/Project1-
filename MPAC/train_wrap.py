...
from MPAC.config_env_pcs import get_env_kwargs, get_env_setup_kwargs
# from MPAC.train_ch import train
...
def train(inputs_dict):
    ...
    # inputs_dict["env_setup_kwargs"]["iso_policy_path"] = "model_iso/iso_policy.zip"
    env_setup_kwargs = inputs_dict["env_setup_kwargs"]  
    env_kwargs = inputs_dict["env_kwargs"]
...
def main():
    ...
    # with mp.get_context('fork').Pool(args.cores) as pool:
    #     log_names = pool.map(train, inputs_list)
    # log_names = [train(i) for i in inputs_list]
