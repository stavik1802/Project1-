...
from MPAC.common.buffers import init_buffer
from MPAC.common.samplers import trajectory_sampler
from MPAC.common.logger import Logger
from MPAC.algs.update_algs import init_update_alg
# stav added 
# import torch
...
class BaseAlg:
    ...
    def _collect_env_data(self, num_timesteps):
        """Collects data by interacting with the training environment and saves reward histograms."""
        ...
        while batch_size_cur < batch_size:
            ...
            rollout_traj, J_all = trajectory_sampler(self.env, self.actor, horizon, gamma=self.gamma)
            (s_traj, a_traj, r_traj, sp_traj, d_traj, c_traj, r_raw_traj) = rollout_traj
            J_tot, Jc_tot, Jc_vec_tot, J_disc, Jc_disc, Jc_vec_disc = J_all

            self.normalizer.update_rms(s_traj, a_traj, r_traj, sp_traj, c_traj, r_raw_traj)
            self.env_data.add(s_traj, a_traj, r_traj, sp_traj, d_traj, c_traj, r_raw_traj)
        ...
        # plot_prefix = f"step_{num_timesteps}"

        # if len(J_tot_all) > 0:
        #     plt.figure()
        #     plt.hist(J_tot_all, bins=30)
        #     plt.title("Trajectory Return Distribution")
        #     plt.xlabel("Return")
        #     plt.ylabel("Count")
        #     plt.grid(True)
        #     plt.tight_layout()
        #     plt.savefig(os.path.join(plot_dir, f"{plot_prefix}_return_hist.png"))
        #     plt.close()

        # if len(all_r_trajs) > 0:
        #     plt.figure()
        #     plt.hist(all_r_trajs, bins=50)
        #     plt.title("Step-wise Reward Distribution")
        #     plt.xlabel("Reward")
        #     plt.ylabel("Count")
        #     plt.grid(True)
        #     plt.tight_layout()
        #     plt.savefig(os.path.join(plot_dir, f"{plot_prefix}_reward_hist.png"))
        #     plt.close()
    ...
    #         batch_size = self.env_batch_size
    #         batch_size_noop = 0
    #     # print(f"[COLLECT] Called with num_timesteps={num_timesteps}, batch_size={self.env_batch_size}", flush=True)
    ...
    #     steps_noop = 0
    #     while batch_size_cur < batch_size:
    #         # print(f"batch cur {batch_size_cur} batch_size {batch_size}")
    #         if self.env_batch_type == 'steps':
    ...
    #             r_raw_traj)
    #         # print(f"[DEBUG] Trajectory length: {len(s_traj)}")
    ...
    #     print(f"Number of steps in buffer: {self.env_data.steps_total}")
    #     print(f"Number of trajectories in buffer: {self.env_data.traj_total}")
    ...
    def train(self, total_timesteps, params):
        ...
        while num_timesteps < total_timesteps:
            ...
            print(f"[TRAIN] Collected {steps_new} steps (noop: {steps_noop}). Total: {num_timesteps}/{total_timesteps}",
                 flush=True)
            # Update
