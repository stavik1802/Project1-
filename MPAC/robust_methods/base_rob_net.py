...
from MPAC.common.nn_utils import transform_features
...
class RobustNet:
    ...
    def _output_normalization(self,delta_rob,out_max):
        ...
        return torch.clamp(delta_rob, min=-out_max, max=out_max)
    ...
    def _calc_sp(self,s,delta,delta_perturb):
        ...
        if self.rob_out_max:
            delta_perturb_clip = self._output_normalization(
                delta_perturb,self.rob_out_max).to(delta.device)
        else:
            delta_perturb_clip = delta_perturb.to(delta.device)

        # apply perturbation
        delta_final = delta * (1.0 + delta_perturb_clip)
